...
public:
	...
	~BPTNode() {
		...
		if (_changed) {
			...
			IndexManager::BM.writeDataToFile("tobedone", _blockNumber, block);
			// TODO: come up with a filename
			// TODO: check serialize order
		}
	}
	...
	void insert_entity(const key_type& k, record_ptr& p) {
		...
		if (_keys.size() == 0 || k > _keys.back()) {
			...
			for (auto i = _keys.begin(), j = _records.begin(); i != _keys.end(), j != _records.end(); i++, j++) {
				if (k < *i) {
					_keys.insert(i, k);
					_records.insert(j, p);
				} else if (k == *i) {
	...
	int split_and_propagate() {
		...
		if (_isLeaf) {
			...
			if (this->is_root()) {
				...
				// TODO: replace "index" with a correct filename
...
public:
	...
	BPlusTree(size_t key_size) :
		...
		_degree((IndexManager::BM.getBlockSize()
			...
			- sizeof(size_t))
			/ (key_size + sizeof(int))),
		_root_block_num(IndexManager::BM.getNewBlockNum()), //TODO: a fileName
	...
	void insert_entity(const key_type& k, const record_ptr& p) {
		...
		if (_root_block_num != leafNode.insert_entity(k, p)) {
			_root_block_num == leafNode.insert_entity(k, p)
				_root = BPTNode<T>(_root_block_num);
		}
	}
	...
	static BPlusTree<T> create(const std::vector<std::pair<T, record_ptr>>& entities, size_t key_size) {
		...
		int root_block_num = IndexManager::BM.getNewBlockNum();
